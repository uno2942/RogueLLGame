...
*/
    ...
    */
    ...
    /**
    * 버프/상태이상에 관련된 메세지를 출력하는 함수
    * 버프가 걸릴 때 사용한다.
    * bool인자로 오버로딩된 함수의 경우 hallucinated 에서 벗어날 시에만 사용.
    */
    ...
    public void MakeBuffMessage(Buff buff)
    {
        ...
        if (buff is Adrenaline) s = "당신의 공격성이 극대화됩니다!";
        else if (buff is Bleed) s = "당신은 피를 흘립니다!";
        else if (buff is Burn) s = "당신에게 불이 붙었습니다!";
        else if (buff is Caffeine) s = "당신은 몸이 날렵해짐을 느낍니다.";
        else if (buff is Full) s = "당신은 포만감을 느낍니다.";
        else if (buff is Hallucinated) s = "당신은 환각에 사로잡혔습니다...";
        else if (buff is Hunger) s = "당신은 배가 고픕니다.";
        else if (buff is Morfin) s = "당신의 감각이 무뎌집니다.";
        else if (buff is Poison) s = "당신은 중독되었습니다!";
        else if (buff is Renewal) s = "당신은 생기를 되찾고 있습니다.";
        else if (buff is Starve) s = "당신은 너무 굶주렸습니다!";
        else if (buff is Stunned) s = "당신은 기절했습니다!";
        else if (buff is VitaminThrown) s = "당신의 공격성이 잠잠해집니다.";
        else
        {
            Debug.Log("정의되지 않은 buff로 MakeBuffMessage를 사용했습니다.");
            return;
        }
        logger.AddLog(s);
    }

    public void MakeBuffMessage(Buff buff, bool buffdeleted) // 환각 벗어날 시 한정으로 사용
    {
        string s = "";
        if(!(buff is Hallucinated) && buffdeleted == true)
        {
            Debug.Log("잘못된 인자를 이용해 MakebuffMessage를 사용했습니다.");
            Debug.Log("buffdeleted가 true인 경우 메세지를 출력하는 경우는 환각에서 벗어날 때 뿐입니다.");
        }
        ...
        else if (buff is Hallucinated)
        {
            if (buffdeleted) s = "당신은 정신을 차렸습니다.";
            else s = "당신은 환각에 사로잡혔습니다...";
        }
        else if (buff is Hunger) s = "당신은 배가 고픕니다.";
        else if (buff is Morfin) s = "당신의 감각이 무뎌집니다.";
        else if (buff is Poison) s = "당신은 중독되었습니다!";
        ...
        {
            Debug.Log("정의되지 않은 buff로 MakeBuffMessage를 발동시켰습니다.");
            return;
        }
        logger.AddLog(s);
    }
